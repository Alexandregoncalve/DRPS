...
module.exports = (pool) => {
  ...
  // GET /api/usuarios/dashboard — VEM ANTES DE /:id
    ...
    try {
      ...
      const { rows } = await pool.query(`
        SELECT
          ...
          COUNT(DISTINCT CASE WHEN r.matriz_risco='Critico' THEN r.avaliacao_id END) as alertas_criticos,
  ...
  router.post('/', autenticar, exigirPapel('admin', 'psicologo'), async (req, res) => {
    ...
    if (!nome || !email || !papel) return res.status(400).json({ erro: 'Campos obrigatorios incompletos' });

    const papeisPermitidos = req.usuario.papel === 'admin'
      ? ['admin', 'psicologo', 'gestor_matriz', 'gestor_filial']
      : ['gestor_matriz', 'gestor_filial'];
    if (!papeisPermitidos.includes(papel)) return res.status(403).json({ erro: 'Sem permissao para este perfil' });
    ...
    try {
      ...
      if (e.code === '23505') return res.status(400).json({ erro: 'E-mail ja cadastrado' });
      console.error(e);
      res.status(500).json({ erro: 'Erro interno' });
    }
  });

  // PATCH /api/usuarios/:id/ativo — VEM ANTES DE PUT /:id
  router.patch('/:id/ativo', autenticar, exigirPapel('admin'), async (req, res) => {
    ...
    if (req.params.id === req.usuario.id) return res.status(400).json({ erro: 'Voce nao pode desabilitar sua propria conta' });
    try {
      const { rows } = await pool.query(
