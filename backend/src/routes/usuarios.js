...
module.exports = (pool) => {
    ...
    try {
      ...
      if (req.usuario.papel === 'admin') {
        ({ rows } = await pool.query(
          `SELECT u.id, u.nome, u.email, u.papel, u.crp, u.criado_em, u.empresa_vinculada_id, e.nome as empresa_nome
           FROM usuarios u LEFT JOIN empresas e ON e.id = u.empresa_vinculada_id
           ORDER BY u.criado_em DESC`
        ));
      } else {
        ({ rows } = await pool.query(
          `SELECT DISTINCT u.id, u.nome, u.email, u.papel, u.crp, u.criado_em, u.empresa_vinculada_id, e.nome as empresa_nome
           FROM usuarios u
  ...
  router.put('/:id', autenticar, exigirPapel('admin', 'psicologo'), async (req, res) => {
    const { nome, email, papel, crp, empresa_vinculada_id, senha } = req.body;
    ...
    try {
      ...
      if (senha && senha.trim()) {
        const hash = await bcrypt.hash(senha, 10);
        query = `UPDATE usuarios SET nome=$1, email=$2, papel=$3, crp=$4, empresa_vinculada_id=$5, senha_hash=$6
                 WHERE id=$7 RETURNING id,nome,email,papel,crp`;
        params = [sanitize(nome), sanitize(email), papel, sanitize(crp) || null, empresa_vinculada_id || null, hash, req.params.id];
      } else {
        query = `UPDATE usuarios SET nome=$1, email=$2, papel=$3, crp=$4, empresa_vinculada_id=$5
                 WHERE id=$6 RETURNING id,nome,email,papel,crp`;
