...
const PAPEIS_PERMITIDOS = {
  admin:     ['psicologo', 'gestor_matriz', 'gestor_filial'], // admin NÃO cria outro admin
  psicologo: ['gestor_matriz', 'gestor_filial'],
};

// Papéis protegidos — nunca aparecem para admin ou psicólogo
const PAPEIS_PROTEGIDOS = ['superadmin', 'admin'];
...
module.exports = (pool) => {
  ...
  router.put('/:id', autenticar, exigirPapel('admin', 'psicologo'), async (req, res) => {
    ...
    try {
      ...
      const alvo = await pool.query(
        'SELECT papel, organizacao_id FROM usuarios WHERE id = $1', [req.params.id]
      );
      if (!alvo.rows.length)
        return res.status(404).json({ erro: 'Usuário não encontrado' });
      if (alvo.rows[0].papel === 'superadmin')
  ...
  router.patch('/:id/ativo', autenticar, exigirPapel('admin'), async (req, res) => {
    ...
    if (req.params.id === req.usuario.id)
      return res.status(400).json({ erro: 'Você não pode desabilitar sua própria conta' });

    try {
      // Nunca desabilita superadmin
      const alvo = await pool.query('SELECT papel FROM usuarios WHERE id = $1', [req.params.id]);
      if (!alvo.rows.length)
        return res.status(404).json({ erro: 'Usuário não encontrado' });
      if (alvo.rows[0].papel === 'superadmin')
        return res.status(403).json({ erro: 'O Super Admin não pode ser desabilitado' });
      ...
      await audit(pool, ativo ? 'USUARIO_REATIVADO' : 'USUARIO_DESABILITADO',
        req.usuario.id, { id: req.params.id }, req);
      res.json(rows[0]);
    } catch (e) {
